...
def get_listings():
    ...
    try:
        ...
        if sort_by == 'price':
            if sort_order == 'desc':
                query = query.order_by(Listing.sale_price.desc().nullslast(), 
                                     Listing.rental_price_weekly.desc().nullslast())
            else:
                query = query.order_by(Listing.sale_price.asc().nullsfirst(), 
            ...
            if sort_order == 'desc':
                ...
                query = query.order_by(Listing.view_count.asc())
        
        # Featured listings first
        query = query.order_by(Listing.is_featured.desc(), query.order_by)
